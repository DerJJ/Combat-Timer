...
- **`perCombatantStats()`**: only `category === "player"` segments, grouped by `overrideOwnerId ?? ownerId` (never by raw `combatantId`). This grouping key is what (a) merges a player's multiple combatants (character + familiar/summon) into one entry, and (b) lets a segment be attributed to a *different* player than whoever's turn it technically was (e.g. an Attack of Opportunity, set via the in-panel player picker). Segments with no resolvable owner (a true, unclaimed NPC turn) are skipped here — they belong to `gmTotalStats()` instead.
- **`gmTotalStats()`**: unclaimed NPC-turn segments (`category === "player" && !isPC && !overrideOwnerId`) plus anything explicitly recategorized to `"dm"`.
- **`categoryTotals()`**: raw per-category sums for `dm`/`team`/`setup`, independent of any specific person.
- **Defeated filter**: any segment with `defeated: true` is excluded from every aggregate function, so an instantly-skipped already-dead combatant can't drag down averages.

## Timing statistics

- **`buildRuns()`**: collapses consecutive segments sharing the same key (`ownerId ?? combatantId`) into contiguous "runs" — a new run always starts on trigger `"turn"` or `"resume"` (never merges across those), so e.g. a solo combatant taking two turns in a row still produces two separate runs.
- **`betweenTurnStats()`**: for each key, the average gap between the end of one run and the start of the next. Additionally, the time before that key's very first run plus the time after its very last run is combined into **one extra synthetic gap** ("wraparound") before averaging. Rationale kept only as a functional note: a boundary fragment alone is structurally biased by seat/turn-order position (correct only for whoever goes last), but the *combination* of both boundary fragments reconstructs something equivalent to one full real gap, and is included as exactly one additional sample. This also means someone with only a single tracked turn still gets a usable average instead of "no data".
- **`absoluteWaitStats()`**: total session span (first segment start to last segment end/now) minus a key's own total active run time. Deliberately does **not** use the wraparound-avoidance trick — this is a sum, not an average, so boundary time is legitimately part of the answer rather than a distorting outlier.
- **`sessionTotalMs()`**: first segment start to last segment end (or now) across the whole selected session — "how long this took overall".
...
- Key panel element ids: `ctp-header`, `ctp-dummy`, `ctp-reset` (dual-purpose new/delete, see below), `ctp-close`, `ctp-status`, `ctp-rows`, `ctp-footer`, `ctp-toolbar` (split button + session tabs, rendered together as equal-width flex items), `ctp-segments`, `ctp-post-bars`, `ctp-post-players`. The small always-on-top reopen button is `ctp-reopen` (not part of the panel's own DOM tree — created separately by `buildReopenButton()`).
