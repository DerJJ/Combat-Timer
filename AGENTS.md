...
```
{
  ...
  actorType: string | null,     // combatant.actor.type ("character"/"npc" in dnd5e/pf2e) - distinguishes a PC from a player-owned summon/pet, used by isRealTurn()
...
- **`isRealTurn(seg)`**: `hasCombatContext(seg) && (!seg.defeated || seg.actorType === "character")`. A defeated PC still gets a real turn (death saves etc. are genuine activity, dnd5e/pf2e `actor.type === "character"`); a defeated NPC's turn is an instant skip — nobody decided anything, so it never counts as "a turn" for averaging, regardless of who its time ends up credited to. `actorType` is captured at segment creation the same way `actorId` is (`combatant?.actor?.type ?? null`); also used by `buildBarsContent()`'s `orderAndShade()` to find a player's own PC among their controlled entities.
...
- **`effectiveOwner(seg, lastLiveOwner)`**: like `resolvedOwner()`, but a defeated NPC's instant-skip inherits whoever owned its last real turn (possibly nobody, i.e. unclaimed) instead of always reading as unclaimed itself. This is what makes a dead monster's leftover time redirect to whoever it was last reassigned to (as that player's `outOfTurnMs`, same mechanism as an Attack of Opportunity) — or, if it was never claimed, land in the GM/Monsters bucket — instead of silently disappearing (S-01). A hidden entity a player's client can't see full data for (no `ownerId`, no `actorType`) resolves exactly the same way an ordinary unclaimed NPC does — there's no separate "unknown" state.
- **`perCombatantStats()`**: only `category === "player"` segments with combat context, grouped by `effectiveOwner()`. Segments with no resolvable owner (a true, unclaimed NPC turn) are skipped here — they belong to `gmTotalStats()`/`npcAggregate()` instead. Splits each owner's time into `inTurnMs` (time during a turn slot they were structurally designated for — see Timing statistics) and `outOfTurnMs` (time reassigned to them from someone else's slot — an Attack of Opportunity, or a redirected defeated NPC); `byEntity` further breaks `inTurnMs`/`outOfTurnMs`/`pausedMs`/`turnCount` down per `combatantId` (keyed by the segment's *own* `combatantId`, not the slot's — an out-of-turn credit's entity is whichever combatant actually did the acting), so a player controlling multiple combatants (character + summons) is shown per-entity in `buildBarsContent()`'s stacked bar instead of only merged into one number. `pausedMs` (both the owner-level and per-entity figure) surfaces as the amber portion of that bar's indicator row. `turnCount` (and `byEntity[].turnCount`) still only increments for `isRealTurn()` slot-openers, so a redirected defeated NPC never inflates anyone's turn count even though its time now counts toward their total.
- **`npcAggregate()`** / **`gmTotalStats()`**: unclaimed NPC-turn segments (`category === "player" && effectiveOwner(seg) === null`) plus, for `gmTotalStats()`, anything explicitly recategorized to `"dm"`. Both use `hasCombatContext()` for the gate and `isRealTurn()` to gate their own `turns`/`turnCount` counters — an unclaimed defeated NPC's instant-skip duration still counts toward `totalMs` (so its time doesn't vanish from the GM's "Monsters total" either), it just never counts as one of the turns the average divides by. `gmTotalStats()` additionally returns `byEntity` (per `combatantId`: `ms`, `turnCount`) so `buildBarsContent()` can render the GM bar as a stack of per-monster segments the same way it does for a player's controlled entities.
- **`categoryTotals()`**: raw per-category sums for `dm`/`team`/`setup`, independent of any specific person. Deliberately has no `defeated` filter at all — unlike the functions above, a `dm`/`team`/`setup` segment isn't about per-owner turn-taking, so it just counts, defeated or not.
...
- `exportSelectedSession()` / `importSessionFromFile()`: operate on whichever session is currently selected (`selectedSession`), not the full persisted blob — exporting downloads a JSON file (`{ exportedAt, world, session, endedAt, segments, currentSegment }`) via a throwaway `Blob`/`<a download>`; importing reads a file picked through a hidden `#ctp-import-file` input, validates only that `segments` is an array, then after a `confirm()` **replaces** that session's data outright (no archiving of what was there). Importing into `"current"` calls `reconcileWithLiveState()` immediately afterward, since the imported `currentSegment` (if any) has no guarantee of matching whatever's actually live right now. All three tabs (`"current"`/`"prev1"`/`"prev2"`) are always selectable, even when `sessionHistory[idx]` doesn't exist yet — an empty archived slot only dims (`hasData: false`), it was never allowed to block the click, since that's the only way to import into a slot that has nothing in it yet. `getSelectedSessionData()` naturally returns `{ segs: [], current: null }` for a missing slot, and `renderPanel()` shows "No data in this slot yet" / "No data yet" text in the status and segment-list sections instead of an empty box.
- `#ctp-segments` shows the *entire* selected session's segment list (no cap), newest first, inside its own `max-height`/`overflow-y:auto` scroll box — `buildSegmentTimeline()` walks the chronological segment list once, inserts a `roundMarkerHTML()` row ("🔄 Round N") wherever an `isTurnStart()` segment's `round` advances, then reverses the whole annotated list for display. A marker only appears when `round` is actually populated, so older/imported data without that field just shows no markers rather than a wrong one.
